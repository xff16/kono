
<h1 align="center">Kono API Gateway</h1>

<p align="center">A lightweight, modular, and high-performance <strong>API Gateway</strong> for modern microservices.</p>

<p align="center">Kono Gateway provides advanced routing, request fan-out, response aggregation, pluggable middleware, and extensibility through custom '.so' plugins.</p> 
<p align="center">It is built with simplicity, performance, and developer-friendly configuration in mind.</p>
